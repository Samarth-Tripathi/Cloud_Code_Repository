	 ...
	 <!-- body-->
	  <div class="row-fluid">
			...
			<!-- breadcrumbs-->
				...
				<?php
					if(isset($_GET['file'])){	//file page
						...
						$user_id = $_GET['user_id'];
						$project = $_GET['project'];
						$query = 'select c.code_id, c.title from code c where c.project_id = '.$project.';';
						$result = mysql_query( $query);
						if (!$result) {
								die('Invalid query: ' . mysql_error());
							}
						...
						echo '</div>';
							...
							$user_id = $_GET['user_id'];
							//$user_id = 1;
							$query = 'select p.project_id, p.title from project p where p.user_id = '.$user_id.';';
							echo '<div class="container" >';
							echo '<h3>Projects</h3>';
		...
		<!--comments content-->
			<div class="span3">
			  <?php 
			   if( isset($_GET['project'])&& !isset($_GET['file'])) comments();
			  ?>
			  <!--Sidebar content-->
			</div>
		</div>
...
function create_project($user_id, $project,$title, $language, $description, $tags){
	...
	$query = 'insert into project(title, description, url , language, user_id, sharing_time ) 
	values ("'.$title.'", "'.$description.'", "data/'.$user_id.'/ ", "'.$language.'", "'.$_SESSION["user_id"].',"'.$time.'")';
	//echo $query;
	mysql_query($query);
...
if (in_array($extension, $allowedExts))
  ...
  if ($_FILES["file"]["error"] > 0)
	...
	if (file_exists("data/".$user_id."/".$project."/".$_FILES["file"]["name"]))
      ...
      //add file to db 
	 // echo $extension;
	   $query  = 'insert into code(	project_id, description,	url ,	title, extension	 ) 
	   values("'.$project.'","","data/'.$user_id.'/'.$project.'/","'.$_FILES["file"]["name"].'","'.$extension.'" )';
	  // echo $query;
	   mysql_query($query);
...
function delete_project($pro_id){
	/*drop all codes in the project
		$query = 'delete from code where project_id='.$pro_id.';';		
		$result = mysql_query($query);
		if (!$result) {
			die('Invalid query: ' . mysql_error());
		}
		...
		/* drop project */
		$query = 'delete from project where project_id = ' .$pro_id.' and user_id = '.$_SESSION['user_id'].';';
		$result = mysql_query($query);
		if (!$result) {
			die('Invalid query: ' . mysql_error());
		}
